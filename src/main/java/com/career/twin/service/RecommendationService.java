...
public class RecommendationService {
    ...
    static {
        ...
        SKILL_RECS.put("HTML_CSS", new SkillRecommendation(
            Arrays.asList("Responsive Web Design Certification (freeCodeCamp)", "Build Responsive Real-World Websites (Udemy)"),
        ...
        SKILL_RECS.put("JavaScript", new SkillRecommendation(
            Arrays.asList("The Complete JavaScript Course (Udemy)", "JavaScript Algorithms and Data Structures (freeCodeCamp)"),
        ...
        SKILL_RECS.put("Machine_Learning", new SkillRecommendation(
            Arrays.asList("Machine Learning Specialization by Andrew Ng (Coursera)", "Intro to Machine Learning (Kaggle)"),
        ...
        SKILL_RECS.put("Deep_Learning", new SkillRecommendation(
            Arrays.asList("Deep Learning Specialization (Coursera)", "Practical Deep Learning for Coders (Fast.ai)"),
            Arrays.asList("Image Classification App (using CNNs)", "AI Chatbot (using simple LSTM/Transformer model)"),
            Arrays.asList("DeepLearning.AI Certification")
        ));
        SKILL_RECS.put("Networking", new SkillRecommendation(
            Arrays.asList("Google IT Support: Bits and Bytes of Networking (Coursera)", "Introduction to Networks (Cisco Networking Academy)"),
        ...
        SKILL_RECS.put("UI_UX_Design", new SkillRecommendation(
            Arrays.asList("Google UX Design Professional Certificate (Coursera)", "UI/UX Design Specialization (Coursera)"),
    ...
    public AnalysisResult analyze(
        ...
        if (cgpa >= 8.5) res.strengths.add("Exceptional academic record (CGPA: " + cgpa + ")");
        if (projectsCount >= 3) res.strengths.add("Solid hands-on project portfolio (" + projectsCount + " items)");
        if (activeSkills.size() >= 5) res.strengths.add("Highly diversified tech stack");

        if (!res.missingSkills.isEmpty()) {
            res.weaknesses.add("Skill gap detected: " + String.join(", ", res.missingSkills));
        }
        if (coding < 60) res.weaknesses.add("Coding proficiency requires additional practice");
        if (communication < 65) res.weaknesses.add("Communication skills could be improved for coding interviews");

        // Calculate radar chart dimension scores
        Map<String, Double> dims = new LinkedHashMap<>();

        boolean holdsCodingSkill = activeSkills.contains("Python") || activeSkills.contains("Java") || activeSkills.contains("C++") || activeSkills.contains("JavaScript");
        double dimCoding = coding * 0.7 + (holdsCodingSkill ? 30.0 : 0.0);
        dims.put("Coding & Logic", Math.min(100.0, Math.round(dimCoding * 10) / 10.0));

        boolean holdsDataSkill = activeSkills.contains("Machine_Learning") || activeSkills.contains("Deep_Learning") || activeSkills.contains("SQL");
        double dimData = analytical * 0.7 + (holdsDataSkill ? 30.0 : 0.0);
        dims.put("Data & Analytics", Math.min(100.0, Math.round(dimData * 10) / 10.0));

        double dimInfra = 30.0 + (activeSkills.contains("Cloud_Computing") ? 25.0 : 0.0) + (activeSkills.contains("Linux") ? 25.0 : 0.0) + (activeSkills.contains("Networking") ? 20.0 : 0.0);
        dims.put("Cloud & Infrastructure", Math.min(100.0, Math.round(dimInfra * 10) / 10.0));

        double dimDesign = 30.0 + (activeSkills.contains("UI_UX_Design") ? 45.0 : 0.0) + (activeSkills.contains("HTML_CSS") ? 25.0 : 0.0);
